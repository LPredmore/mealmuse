...
export default function EditMemberDialog({ open, member, onClose, onSave }) {
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        adventurousness_level: member.adventurousness_level || "moderate",
        other_preferences: member.other_preferences || ""
      });
    }
  }, [member]);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="backdrop-blur-xl bg-black/40 border border-white/20 rounded-3xl max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-white text-2xl font-bold">Edit Family Member</DialogTitle>
        </DialogHeader>
